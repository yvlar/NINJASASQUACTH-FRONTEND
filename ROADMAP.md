...
| D23 | 🔴→✅ | (Sprint 11.1) **Fusion sans déploiement réel** : le code des Sprints 11-12 était présent mais **les migrations `newsletter_subscribers` et `deploy_rebuilds` n'étaient PAS appliquées** (tables inexistantes) et **aucune Edge Function n'était déployée** — la newsletter et le rebuild étaient donc totalement non fonctionnels en prod. De plus le HTML pré-rendu était **remplacé** (`createRoot`) au lieu d'être hydraté, et le rewrite Vercel catch-all servait une soft-404 en 200 | ✅ Sprint 11.1 — migrations + RPC + `deploy_rebuilds` appliquées, Edge Functions déployées (ACTIVE, fail-closed vérifié en live), `hydrateRoot` + amorce sérialisée, `vercel.json` (redirect / + rewrites limités à /admin + vraie 404) |
| D24 | 🟠 | (Sprint 11.1) **Jeu « Mario » publié sans slug** : la seule ligne `games` publiée n'a pas de `slug` → aucune fiche partageable générable. Capté par le garde `REQUIRE_PRERENDER_GAMES` (le build de prod échoue clair). Données de test manifestes (titre « Mario », description « Mario Bros. ») | **Action utilisateur** : donner un slug + un vrai contenu au jeu, ou le repasser en brouillon |
| D25 | 🟡 | (Sprint 11.1) **Désinscription newsletter non automatique** : le libellé promettait « se désinscrire à tout moment » sans mécanisme. Corrigé en libellé honnête (« en nous écrivant ») + procédure manuelle admin (statut `unsubscribed`) documentée. Une Edge Function `unsubscribe-newsletter` à jeton signé reste une évolution possible | **Évolution possible** : lien de désinscription à jeton signé (non réversible, expiration) |
