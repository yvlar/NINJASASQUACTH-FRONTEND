...
export default function GameHero({ game }: { game: GameRow }) {
  ...
  return (
    <header className="mx-auto grid max-w-6xl grid-cols-[1fr] gap-8 px-4 py-8 md:grid-cols-[repeat(2,1fr)]">
      <div>
        {/* Toujours un visuel : la photo réelle, ou un repli de marque
            thématisé (plus de colonne vide). Décoratif : le <h1> est adjacent. */}
        <GameArtwork
          imageUrl={game.image_url}
          title={title}
          themeKey={game.theme_key}
          decorative
          className="h-96 w-full rounded-lg shadow-[0_10px_15px_-3px_rgba(0,0,0,0.1)]"
        />
      </div>
