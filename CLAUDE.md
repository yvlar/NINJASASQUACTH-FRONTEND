...
**Error boundary.** `src/components/ErrorBoundary/` (class `ErrorBoundary.tsx` + functional `ErrorFallback.tsx` split for `react-refresh`) wraps `AppRoutes` inside `LanguageProvider` so the fallback reads `t("errors.boundary.*")`. It catches render exceptions (no more white screen) and logs to `console.error`. Since Sprint 11 (Partie B) `lib/supabase.ts` no longer throws at import when env is missing — it exports `isSupabaseConfigured` and a fallback client on a non-resolvable `.invalid` URL, so a missing config degrades to a visible marketing site + a localized catalogue error instead of a white screen (locked by `supabase-client.test.ts` and `degraded-config.test.tsx`).
...
**Icons.** Pulled from `lucide-react` as named imports (e.g. `Menu`/`X` in `Header`, `Leaf` in `About`/`Footer`/`GameCard`, `Users`/`Clock`/`Star` in the Games components, `Facebook`/`Youtube`/`Linkedin`/`Mail` in `SocialLinks`/`Contact`). Decorative icons carry `aria-hidden`; the accessible name lives on the link/button.

**Social links + featured game.** `src/data/socialLinks.ts` is the single source of the real social URLs (Facebook, YouTube, LinkedIn — no `href="#"`, no Kickstarter without a public URL); the shared `SocialLinks` component (`src/components/layout/SocialLinks/`) renders them accessibly (new tab, `rel="noreferrer"`, localized `aria-label`, decorative icons hidden). `src/utils/featuredGame.ts` (`selectFeaturedGame`, `isEnglishOnly`) is pure: the home Hero **reads its featured game from Supabase** (Heroes Rising prioritized), never hardcoded, and the « contenu en anglais » badge is derived from `game_languages`. `src/data/gameThemes.ts` maps the controlled `theme_key` to **static, exhaustive, typed** accent variants (`themeForKey` falls back to the brand theme) — Tailwind classes are never built dynamically from a Supabase value; sub-palettes are game-local accents only, never the header/footer/main background.

## Styling

**Tailwind CSS v4** utility classes in the JSX, plus one entry stylesheet `src/styles/global.css` (imported once in `main.tsx`) that does `@import "tailwindcss"`, declares the brand palette + fonts in an `@theme` block, and re-adds the few base rules the preflight doesn't cover (`@layer base`: `scroll-behavior` + `prefers-reduced-motion`, the body font `--font-body` + `line-height: 1.6` + `text-align: left`, `--font-brand` on headings at weight 400). No per-component stylesheet.

The brand palette + typography live once, in the `@theme` block of `global.css` — each token generates utilities **and** stays available as `var(--…)`. Always use the utility (or the var inside an arbitrary value), **never a raw hex** outside `@theme` (Sprint-10 rebrand):
- `--color-cream` `#fbf8e9` → `bg-cream` (main background, text on dark)
- `--color-forest` `#1f3a1f` → `bg-forest`/`text-forest` (vert forêt — CTAs, eco/success accents, focus rings)
- `--color-roux` `#a0522d` → `text-roux` (Sasquatch roux — brand headings, accents)
- `--color-charcoal` `#2b2420` → `text-charcoal` (charbon — body text, dark surfaces, footer)
- `--color-error` `#b3261e` → `text-error` (form errors)
- **Heroes Rising** (game-local accents only): `--color-hr-gold` `#ffd400`, `--color-hr-orange` `#ff7a00`, `--color-hr-red` `#d4213a`
- **Burgle Jack** (game-local accents only): `--color-bj-yellow` `#ffe800`, `--color-bj-black` `#000000`

Palette-derived tints use opacity modifiers (`bg-roux/5`, `bg-forest/10`); non-token values (shadows, exact sizes) use arbitrary-value classes (`shadow-[…]`, `max-w-[60rem]`, `text-[2.5rem]`) — the raw value is allowed **only inside the brackets**. Typography (loaded via `<link>` in `index.html`): headings get **Alfa Slab One** (`--font-brand`, single weight — no forced `font-weight`) from `@layer base`; body is **Atkinson Hyperlegible** (`--font-body`, chosen for legibility); **Black Ops One** (`--font-accent`) is reserved for a punctual Heroes Rising accent. Never use the display font for long paragraphs; interline ≥ 1.5, left-aligned, no justified text, no long uppercase runs. Default Tailwind breakpoints: `sm:` 640px, `md:` 768px, `lg:` 1024px. Interactive elements carry a `focus-visible:` ring and animated ones a `motion-reduce:` variant.
